defmodule ElixirKatas.VendingMachine.State do
  defstruct current_amount: 0.00, coin_return: 0.00, msg: "", stock: %{"COLA" => 10, "CHIPS" => 10, "CANDY" => 10}
end
...
defmodule ElixirKatas.VendingMachine do
  ...
  def select_product(state, product) do
    cond do
      ...
      state[:current_amount] == @vending_machine_products[product] ->
        %{state | :msg => @thank_you, :current_amount => 0.00, :stock => %{state[:stock] | product => state[:stock][product] - 1}}
      state[:current_amount] == 0 ->
        %{state | :msg => @insert_coin_msg, :current_amount => 0.00}
      state[:current_amount] < @vending_machine_products[product] ->
        %{state | :msg => "PRICE #{@vending_machine_products[product]}"}
      state[:current_amount] > @vending_machine_products[product] ->
        %{state | :msg => "", :current_amount => 0.00, :coin_return => Float.round(state[:current_amount] - @vending_machine_products[product], 2), :stock => %{state[:stock] | product => state[:stock][product] - 1}}
      true -> 
        state
    end
  end
  ...
  def return_coins(state) do
      %{state | :coin_return => state[:current_amount], :current_amount => 0.00, :msg => @insert_coin_msg}
  end
